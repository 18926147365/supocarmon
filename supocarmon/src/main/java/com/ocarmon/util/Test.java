...
public class Test {
	...
	public static void main(String[] args) {
		...
		// ddd(urlToken);
		// String
		// ipString=getIp("http://120.25.150.39:8081/index.php/api/entry?method=proxyServer.ipinfolist&quantity=&province=%E5%B9%BF%E4%B8%9C%E7%9C%81&city=%E5%B9%BF%E5%B7%9E%E5%B8%82&anonymous=1&ms=1&service=0&protocol=1&wdsy=on&distinct=true&format=json&separator=1&separator_txt=");
	...
	public static void connDB() {
		try {
			...
			DBCollection course = db.getCollection("bigangle");//
			DBCursor cursor = course.find();
			Iterator<DBObject> dboi = cursor.iterator();
			String str="";
			while (dboi.hasNext()) {
				DBObject dbo = dboi.next();
				String ip=(String) dbo.get("ip");
				int type=(int)dbo.get("type");
				String info="";
				if(type==1) {
					info="访问活动";
				}
				...
				String createTime=(String) dbo.get("createTime");
				str+="(1,'"+info+"',"+type+",'1','"+ip+"','"+createTime+"'),";
